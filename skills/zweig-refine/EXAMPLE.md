...
Read the file, then cut every removable sentence and phrase with real Edit calls: throat-clearing ("We wanted to let you know that"), restated framing ("which is a change from how it used to work before this change was made"), and hedge words ("approximately", "or so", "on average").
...
One edit made — the loop continues.
...
One edit, addressing the flagged gap, then stop — no re-running the loop or the gate:
